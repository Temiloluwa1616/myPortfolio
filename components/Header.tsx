...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 w-full bg-gray-900/90 backdrop-blur-lg border-b border-gray-700/50 z-50 shadow-lg">
          ...
          <div className="hidden md:flex space-x-8">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="text-gray-300 hover:text-white relative group transition-all duration-300"
              >
                {item.name}
